...
public interface AddressControllerDoc {
    ...
    @Operation(summary = "List all address of the client.", description = "Retrive a ArrayList with all address of the database that are related to a client (send the client ID in the parameters) in DTOs (the ID in the DTO can be used to access the entity, if necessary) on a ResponseEntity.")
    @ApiResponses(
            value = {
    ...
    @Operation(summary = "Save a new address.", description = "Save a new address on the database. You can send the address information by the AddressInputDTO and th owner of the address on the parameters. The owner must be registered on the sistem as a client.")
    @ApiResponses(
            value = {
